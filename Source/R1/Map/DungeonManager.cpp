...
void ADungeonManager::CompleteRoom()
{
	...
	if (ClearCondition == ER1RoomClearCondition::KillBoss && PortalClass)
	{
		// 마지막 층이든 아니든 포탈은 항상 생성한다.
		// 다음 층으로 갈지 엔딩 레벨로 갈지는 포탈(AR1Portal)이 판단한다.
		FVector PortalLocation = GetActorLocation() + PortalSpawnOffset;
